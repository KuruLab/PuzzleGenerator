...
public class PuzzleEvaluation {
    ...
    public void solve(Graph original, PuzzleIndividual puzzle){
        ...
        puzzle.getSolution().add(current.getId());
        while(!targets.isEmpty()){
            ...
            for (Node step : path.getNodePath()) {
                if(step.equals(path.getRoot())){
                    ...
                    puzzle.getSolution().add(step.getId());
                }
            }
    ...
    public double[] fitness(Graph original, PuzzleIndividual puzzle, boolean debug) {
        PuzzleDecoder pdec = new PuzzleDecoder(original);
        Graph graph = pdec.decode(puzzle, false);
        ...
        fitness[1] = Math.abs(nonlinearity - PuzzleConfig.idealNonLinearity);
