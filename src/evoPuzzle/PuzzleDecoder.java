...
public class PuzzleDecoder {
    ...
    public PuzzleDecoder(Graph original) {
        this.graph = original;
    ...
    public PuzzleDecoder(Graph original, int seed) {
        this.graph = original;
    ...
    public PuzzleIndividual encode(){
    ...
    public Graph decode(PuzzleIndividual puzzleInd, boolean debug) {
        if(seed == -1){
            seed = puzzleInd.toString().hashCode();
        ...
        //int attempt = 0;
        PuzzleConfig.defaultMaxKeys = (int) ((graph.getNodeCount() / 5.0) - 1);
        (new PuzzleUtil()).initializeGraphSymbols(graph);
        (new PuzzleUtil()).initializeGraphConditions(graph);
        (new PuzzleUtil()).initializeGraphIntensity(graph);
        ...
        if (debug) {
            System.out.println("Rooms per lock defined as: " + roomsPerLock +
                               " Max Keys: " + PuzzleConfig.defaultMaxKeys);
        }
    ...
    public void fixBossKey(KeyLevelRoomMapping levels, PuzzleIndividual individual){
        ...
        if (oldKeyLevel != newKeyLevel) {
            ...
            Symbol bossKey = new Symbol(newKeyLevel);
